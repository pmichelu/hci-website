...
export default function EditProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/newsletters/lists").then((r) => r.json()),
    ]).then(([data, newsletters]) => {
      setName(data.name || "")
      setSlug(data.slug || "")
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Newsletter Signup</label>
          <select
            value={newsletterSlug}
            onChange={(e) => setNewsletterSlug(e.target.value)}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)] focus:border-transparent outline-none bg-white"
          >
            <option value="">None</option>
            {availableNewsletters.map((nl) => (
              <option key={nl.slug} value={nl.slug}>{nl.name}</option>
            ))}
          </select>
